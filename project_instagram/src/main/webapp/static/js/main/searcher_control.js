...
searcher_input.oninput = (event) => {
	...
	$.ajax({
		...
		success: function (data) {
			...
			result_wrapper.querySelector(".results").innerHTML = "";
...
function makeResultBox(latest_searches) {
		...
		for(let i = 0; i < latest_searches.length; i++) {
			const div = document.createElement("div");
			div.className = "result";
			div.innerHTML = `<img src="/static/images/${latest_searches[i].has_profile_image == true ? 'user_profile_images/' + latest_searches[i].file_name : 'basic_profile_image.jpg'}" alt='유저 프로필 이미지'>
											<div class="name-wrapper"></div>
